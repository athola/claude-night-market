...
- **imbue hard vow hooks**: Three PreToolUse hooks that enforce
  constraints mechanically rather than through willpower.
  `vow_no_emoji_commits.py` blocks emoji in git commit messages.
  `vow_no_ai_attribution.py` blocks AI attribution strings
  ("Co-authored-by: Claude", "Generated by Claude", etc.).
  `vow_bounded_reads.py` warns when discovery read budget (15
  reads per session) is exceeded. All three default to shadow
  mode (warn only); set `VOW_SHADOW_MODE=0` for full blocking.
  Includes 116 unit tests covering all code paths and the
  modern `hookSpecificOutput.permissionDecision` protocol.

### Fixed

- **imbue hook modernization**: Replaced deprecated top-level
  `{"decision": ..., "reason": ...}` output format in three new
  vow hooks with the current `hookSpecificOutput.permissionDecision`
  / `permissionDecisionReason` structure required by Claude Code.
- **imbue test lint**: Added `check=False` to
  `subprocess.run` call in `test_scope_guard_hook.py` to fix
  `PLW1510` ruff warning.
...
- **imbue justify**: `/justify` command and skill that
  audits changes for AI additive bias. Scores changes
  on line ratio, new files, test tampering, and new
  abstractions. Enforces Iron Law compliance and weights
