...
def initiate_oauth(service: str) -> AuthStatus:
    ...
    result = subprocess.run([service, "auth", "login"], capture_output=True)

    if result.returncode == 0:
        return AuthStatus(authenticated=True, message="OAuth completed successfully")
...
def refresh_token(service: str) -> AuthStatus:
    ...
    result = subprocess.run([service, "token", "refresh"], capture_output=True)
