...
description: Evaluate codebase architecture against ADRs, coupling rules, and team guardrails. Use for principal-level assessments before approving major work.
...
Principal-level architecture assessment against ADRs and design principles.
...
- **`modules/adr-audit.md`** (~400 tokens) - Load for ADR verification, new decision documentation, or governance compliance checks
- **`modules/coupling-analysis.md`** (~450 tokens) - Load for dependency analysis, boundary violations, or refactoring reviews
- **`modules/principle-checks.md`** (~500 tokens) - Load for code quality checks, security review, or performance validation

Load all modules for comprehensive architecture reviews. For focused reviews, load only relevant modules.
...
- Verify required sections (Title, Status, Context, Decision, Alternatives, Consequences)
- Check status flow: Proposed → Reviewed → Accepted
...
- Law of Demeter (train wreck detection)
- Anti-slop patterns (overengineering, generic naming)
- Security (input validation, least privilege, error handling)
- Performance (N+1 queries, caching, budgets)
