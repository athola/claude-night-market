...
This document is a per-version timeline of Claude Code features that
matter to plugin authors, newest first. It is written for a practitioner
who knows Claude Code but not this repository.
...
- ✅ Bash `#` permission, "Don't Ask Again" pipes, slash
  ...
  (session reaping, message batching, JWT refresh,
  WebSocket recovery), soft-hidden commands, VSCode
  gitignore commas.
...
- ✅ **1M Context Window Default for Max/Team/Enterprise**:
  Opus 4.6. No extra usage required. Opt out with
...
- ✅ **Effort Levels Simplified**: Reduced from 4 levels (`low`,
  ...
  default. Brief notification replaces persistent icon. `max` was
  removed for Opus 4.6 and reintroduced for Opus 4.7 in 2.1.111,
  alongside `xhigh`.
