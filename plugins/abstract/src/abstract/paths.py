"""Resolve the directories this plugin reads and writes.

These helpers lived in ``utils``, which imports PyYAML and the
frontmatter processor to do its own work. That made three pure path
...
``utils`` re-exports all three, so every existing caller is unchanged.
