...
class TestPruneCheck:
    ...
    def _fresh_palace(
        ...
        _write_palace(temp_palaces_dir, _make_palace(
            "fresh-palace", "Fresh Palace",
            {"entry1": {
                "label": "Recent Entry",
                "query": "recent topic",
                "timestamp": now,
    ...
    def _stale_palace(
        ...
        old_time = (
            datetime.now() - timedelta(days=STALE_DAYS + 30)
        ).isoformat()
        now = datetime.now().isoformat()
        _write_palace(temp_palaces_dir, _make_palace(
            "stale-palace", "Stale Palace",
            {
                "old-entry": {
                    "label": "Old Entry",
                    "query": "old topic",
    ...
    def _low_quality_palace(
        ...
        now = datetime.now().isoformat()
        _write_palace(temp_palaces_dir, _make_palace(
            "lowq-palace", "Low Quality Palace",
            {
                "weak-entry": {
                    "label": "Weak Entry",
                    "query": "weak topic",
