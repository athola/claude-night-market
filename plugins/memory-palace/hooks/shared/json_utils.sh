...
#
# VENDORED COPIES (D-01): Each plugin keeps a byte-identical copy at
# its hooks/shared/json_utils.sh so hooks can source via
# ${CLAUDE_PLUGIN_ROOT}/hooks/shared/json_utils.sh from the Claude
# Code plugin cache. The drift checker (scripts/shared/check-json-
# utils-drift.sh) verifies the vendored copies match this canonical.
# Vendored locations:
...
#
# Usage:
#   source "${BASH_SOURCE[0]%/*}/../../scripts/shared/json_utils.sh"
...
get_json_field() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    value=$(printf '%s\n' "${json}" | jq -r --arg f "${field}" '.[$f] // empty' 2>/dev/null || printf '\n')
  elif printf '%s\n' "test" | grep -oP '\d+' >/dev/null 2>&1; then
    # Tier 2: GNU grep with Perl regex
    value=$(printf '%s\n' "${json}" | grep -oP "\"${field}\"\\s*:\\s*\"\\K[^\"]+" 2>/dev/null || printf '\n')
  else
    # Tier 3: POSIX-compliant sed (universal fallback)
    value=$(printf '%s\n' "${json}" | sed -n "s/.*\"${field}\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p" 2>/dev/null || printf '\n')
  fi
...
escape_for_json() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    case "${_JSON_ESCAPE_WARN:-0}" in
      0) printf '%s\n' "[WARN] jq not found, using bash fallback for JSON escaping. Install jq for better performance." >&2 && export _JSON_ESCAPE_WARN=1 ;;
    esac
