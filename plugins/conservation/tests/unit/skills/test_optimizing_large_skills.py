...
"""
    ...
    @pytest.mark.bdd
    @pytest.mark.unit
    def test_optimizing_large_skills_creates_required_todowrite_items(self, mock_todo_write):
    ...
    def test_skill_analysis_identifies_optimization_candidates(self):
        ...
        optimization_candidates.sort(key=lambda x: (
    ...
    def test_modularization_assessment_identifies_breakdown_opportunities(self):
        ...
        large_skill_content = {
            ...
            "total_lines": 950,
            "complexity_indicators": ["multiple_formats", "cross_language", "deep_analysis"]
        }

        # Act - assess modularization opportunities
        modularization_analysis = {
            "total_complexity": len(large_skill_content["sections"]) + len(large_skill_content["complexity_indicators"]),
            "identified_modules": [],
            "breakdown_points": [],
            "dependency_graph": {}
        }
        ...
        for section in large_skill_content["sections"]:
            ...
            should_be_module = (
                section["lines"] > 100 or  # Large sections
                len(section["dependencies"]) == 0 or  # Independent sections
                "scanning" in section["responsibility"] or  # Specialized functionality
                "generation" in section["responsibility"]  # Output functionality
            )
        ...
        modularization_analysis["benefits"] = {
            ...
            "maintainability_improvement": "High" if modularization_ratio > 0.7 else "Medium",
            "reusability_potential": len([m for m in modularization_analysis["identified_modules"] if m["module_type"] == "independent"])
        }
    ...
    def test_performance_profiling_identifies_bottlenecks(self, mock_claude_tools):
        ...
        for operation in skill_execution_data:
            ...
            performance_analysis["total_memory_peak"] = max(performance_analysis["total_memory_peak"], total_memory)
            performance_analysis["total_token_usage"] += total_tokens

            # Identify bottlenecks (operations taking > 20% of total time)
            time_percentage = (total_time / sum(op["execution_time"] * op["frequency"] for op in skill_execution_data)) * 100
            ...
            if total_tokens > 1000 or total_memory > 300:
                performance_analysis["optimization_targets"].append({
                    ...
                    "usage_amount": total_tokens if total_tokens > 1000 else total_memory,
                    "optimization_potential": "High" if total_tokens > 2000 else "Medium"
                })

        # Sort bottlenecks by impact
        performance_analysis["bottlenecks"].sort(key=lambda x: x["impact_percentage"], reverse=True)

        # Assert
        assert performance_analysis["total_execution_time"] > 30  # Should be significant
        ...
        data_processing = next((b for b in performance_analysis["bottlenecks"] if b["operation"] == "data_processing"), None)
        assert data_processing is not None
        assert data_processing["impact_percentage"] > 20

        # Verify optimization targets
        token_optimizations = [t for t in performance_analysis["optimization_targets"] if t["resource_type"] == "tokens"]
    ...
    def test_optimization_recommendations_prioritize_high_impact_changes(self):
        ...
        for bottleneck in analysis_results["performance_bottlenecks"]:
            priority_score = (bottleneck["estimated_improvement"] *
        ...
        for module in analysis_results["modularization_opportunities"]:
            ...
            priority_score = (module["complexity_reduction"] * reusability_factor) / \
        ...
        for inefficiency in analysis_results["resource_inefficiencies"]:
            ...
            priority_score = (inefficiency["potential_savings"] * urgency_factor) / \
    ...
    def test_validation_testing_measures_optimization_effectiveness(self):
        ...
        post_optimization_metrics = {
            "skill_load_time": baseline_metrics["skill_load_time"] * (1 - modularization_improvement["load_time_improvement"]),
            "memory_usage_mb": baseline_metrics["memory_usage_mb"] * (1 - max(modularization_improvement["memory_efficiency"], resource_improvement["memory_reduction"])),
            "token_efficiency": baseline_metrics["token_efficiency"] * (1 + performance_improvement["token_efficiency"] + resource_improvement["token_savings"]),
            "execution_success_rate": min(baseline_metrics["execution_success_rate"] + performance_improvement["success_rate"], 1.0),
            "user_satisfaction_score": modularization_improvement["maintainability_score"]
        }

        # Calculate improvements
        validation_results = {
            "load_time_improvement_percentage": ((baseline_metrics["skill_load_time"] - post_optimization_metrics["skill_load_time"]) / baseline_metrics["skill_load_time"]) * 100,
            "memory_savings_percentage": ((baseline_metrics["memory_usage_mb"] - post_optimization_metrics["memory_usage_mb"]) / baseline_metrics["memory_usage_mb"]) * 100,
            "token_efficiency_improvement": ((post_optimization_metrics["token_efficiency"] - baseline_metrics["token_efficiency"]) / baseline_metrics["token_efficiency"]) * 100,
            "success_rate_improvement": ((post_optimization_metrics["execution_success_rate"] - baseline_metrics["execution_success_rate"]) / baseline_metrics["execution_success_rate"]) * 100,
            "satisfaction_improvement": post_optimization_metrics["user_satisfaction_score"] - baseline_metrics["user_satisfaction_score"]
        }

        # Determine success criteria
        success_criteria = {
            "load_time_target_met": validation_results["load_time_improvement_percentage"] >= 20,
            "memory_target_met": validation_results["memory_savings_percentage"] >= 15,
            "token_efficiency_target_met": validation_results["token_efficiency_improvement"] >= 25,
            "overall_success": all([
