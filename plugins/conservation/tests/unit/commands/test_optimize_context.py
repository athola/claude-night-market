...
class TestOptimizeContextCommand:
    ...
    @pytest.mark.bdd
    @pytest.mark.unit
    def test_command_orchestrates_context_optimization_workflow(self, mock_optimize_context_command, mock_claude_tools):
        ...
        for skill_name in mock_optimize_context_command["skills_orchestrated"]:
            skill_result = skill_execution_results.get(skill_name, {"status": "skipped"})

            workflow_execution["skills_executed"].append(skill_name)
            workflow_execution["results"][skill_name] = skill_result

        # Calculate overall results
        successful_skills = [skill for skill, result in workflow_execution["results"].items()
    ...
    @pytest.mark.bdd
    @pytest.mark.unit
    def test_command_adapts_optimization_based_on_aggressiveness(self, mock_claude_tools):
        ...
        aggressiveness_levels = [
            ...
            },
            {
                ...
                "expected_strategies": ["context_compression", "prompt_optimization", "targeted_removal"],
            ...
            },
            {
                ...
                "expected_strategies": ["deep_compression", "aggressive_pruning", "delegation"],
                "risk_tolerance": "high",
    ...
    def test_command_handles_different_target_types(self, mock_claude_tools):
        ...
        target_scenarios = [
            ...
            },
            {
                ...
                "type": "session",
                "expected_strategies": ["session_optimization", "context_window_management"],
                "scope": "entire_session"
            },
            {
                "target": "docs/api.md",
                "type": "documentation",
                "expected_strategies": ["documentation_compression", "content_summarization"],
        ...
        for scenario in target_scenarios:
            ...
            overall_effectiveness = sum(s["effectiveness"] for s in optimization_approach["strategies"]) / len(optimization_approach["strategies"])
        ...
        for result in target_optimization_results:
            ...
            assert result["expected_effectiveness"] >= 0.5  # Should be reasonably effective
    ...
    def test_command_provides_comprehensive_feedback(self, mock_claude_tools):
        ...
        user_feedback["achievements"]["context_reduction_percentage"] = (context_savings / context_result["tokens_before"]) * 100
        ...
        if user_feedback["achievements"]["context_reduction_percentage"] > 15:
            user_feedback["recommendations"].append("Consider similar optimization for related files")

        if token_result["quota_after"]["remaining"] < 20000:
            user_feedback["recommendations"].append("Monitor weekly quota usage closely")

        if optimization_results["aggressiveness"] == "moderate":
            user_feedback["recommendations"].append("Consider aggressive optimization for larger files")
