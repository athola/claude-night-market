"""Test quality evaluation functions for the testing guide.

C9 (PR #470 review, issue #485): the prior shape was a stateless
``TestQualityMixin`` class composed into ``TestingGuideSkill`` via
multiple inheritance. The methods carried no state, never called
``self``, and were interchangeable with module-level functions. They
are now module-level functions; the skill class delegates to them.

Returns are still typed via the ``QualityAspects`` /
``QualityAssessment`` / ``QualityEnvelope`` ``TypedDict``s so static
checkers narrow the dict keys callers rely on.
"""
...
def evaluate_test_quality(code: str) -> dict[str, Any]:
...
def evaluate_maintainability(code: str) -> dict[str, Any]:
...
def validate_async_testing(code: str) -> dict[str, Any]:
    ...
    validation["async_test_count"] = len(re.findall(r"async\s+def\s+test_\w+", code))
