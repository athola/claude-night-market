...
class TestSkippedItemsAreCountable:
    """Feature: a parser that drops items can say how many (PR #839 review).

    Three items yielding one Finding with no record of the two dropped
    is indistinguishable from a one-item response. If the shape drifts
    (``url`` becomes ``link``), ten real results yield zero findings and
    the channel reads ``empty``.
    """

    @pytest.mark.unit
    def test_dropped_collects_one_reason_per_skipped_item(self) -> None:
        dropped: list[str] = []
        findings = parse_you_mcp_result(
            [{"url": "https://a"}, "junk", {"title": "no url"}],
            "topic",
            dropped=dropped,
        )
        assert len(findings) == 1
        assert len(dropped) == 2
        assert any("not an object" in reason for reason in dropped)
        assert any("no url" in reason for reason in dropped)

    @pytest.mark.unit
    def test_without_dropped_the_contract_is_unchanged(self) -> None:
        assert len(parse_you_mcp_result([{"url": "https://a"}, "junk"], "topic")) == 1
