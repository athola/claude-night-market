...
class TestSpeckitCommands:
    ...
    class TestSpecifyCommand:
        ...
        def test_branch_name_generation(self, sample_feature_description):
            ...
            keywords = ["user", "authentication", "email", "password", "role", "access", "control"]
            found_keywords = [kw for kw in keywords if kw in feature_desc.lower()]

            assert len(found_keywords) >= 2, "Should identify at least 2 keywords for branch name"
            ...
            assert any(candidate.replace("-", " ") in feature_desc.lower() for candidate in short_name_candidates)
        ...
        @patch('subprocess.run')
        def test_create_feature_script_execution(self, mock_run, sample_feature_description):
        ...
        def test_feature_description_validation(self):
            ...
            for desc in valid_descriptions:
                assert len(desc.strip()) >= 10, f"Valid description should be substantial: {desc}"

            for desc in invalid_descriptions:
                if desc.strip():  # If not empty/whitespace
                    assert len(desc.strip()) < 10 or len(desc) > 500, f"Invalid description should be caught: {desc}"
    ...
    class TestPlanCommand:
        ...
        def test_specification_parsing(self, sample_spec_content):
            ...
            assert "Functional Requirements" in sections, "Should parse Functional Requirements"
        ...
        def test_plan_generation_from_spec(self, sample_spec_content):
            ...
            assert len(planning_points) >= 2, f"Should generate planning points, got: {planning_points}"
        ...
        def test_dependency_validation(self, sample_task_list):
            ...
            if setup_phase:
                setup_tasks_without_deps = [
                    task["id"] for task in setup_phase["tasks"]
                    if not task["dependencies"]
                ]
                assert all(task_id in ready_tasks for task_id in setup_tasks_without_deps), "Setup tasks without deps should be ready"
    ...
    class TestChecklistCommand:
        ...
        def test_completion_checklist_generation(self, sample_spec_content, sample_task_list):
            ...
            assert len(checklist_items) >= 3, f"Should generate checklist items, got: {checklist_items}"
        ...
        def test_verification_criteria_check(self, sample_spec_content):
            ...
            assert len(success_section.strip()) > 0, "Should have success criteria to verify"

            # Check for verifiable criteria
            verifiable_patterns = ["can", "will", "should", "must"]
            has_verifiable = any(pattern in success_section.lower() for pattern in verifiable_patterns)
