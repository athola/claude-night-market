...
class TestFrontmatterValidation:
    ...
    def test_spec_writing_skill_frontmatter(self, temp_skill_files):
        ...
        assert 'category: specification' in frontmatter, "Category should be specification"
    ...
    def test_orchestrator_skill_frontmatter(self, temp_skill_files):
        ...
        # Validate field values
        assert 'name: speckit-orchestrator' in frontmatter, "Name should match directory"
        assert 'category: workflow-orchestration' in frontmatter, "Category should be workflow"
    ...
    def test_frontmatter_yaml_validity(self, temp_skill_files):
        ...
        for skill_name in skill_dirs:
            ...
            try:
                ...
                assert parsed is not None, f"Frontmatter should be parseable for {skill_name}"
    ...
    def test_optional_frontmatter_fields(self, temp_skill_files):
        ...
        optional_fields = ['tags', 'dependencies', 'tools', 'complexity', 'estimated_tokens']
        ...
        for field in optional_fields:
            if field in parsed:
                ...
                if field in ['tags', 'dependencies', 'tools']:
                    ...
                    assert isinstance(value, (int, str)), f"{field} should be a number or string"
    ...
    def test_consistent_frontmatter_structure(self, temp_skill_files):
        ...
        assert expected_common.issubset(common_fields), f"Skills should have common fields: {expected_common}"

    def test_plugin_manifest_validation(self, sample_plugin_manifest):
        """Test plugin manifest follows expected structure."""
        required_fields = ['name', 'version', 'description', 'commands', 'skills', 'agents']

        for field in required_fields:
            assert field in sample_plugin_manifest, f"Plugin manifest missing required field: {field}"

        # Validate field types
        assert isinstance(sample_plugin_manifest['commands'], list), "Commands should be a list"
        assert isinstance(sample_plugin_manifest['skills'], list), "Skills should be a list"
        assert isinstance(sample_plugin_manifest['agents'], list), "Agents should be a list"

        # Check version format
        version_pattern = r'^\d+\.\d+\.\d+$'
        assert re.match(version_pattern, sample_plugin_manifest['version']), f"Invalid version format: {sample_plugin_manifest['version']}"

    def test_skill_references_in_manifest(self, sample_plugin_manifest, temp_skill_files):
        ...
        for skill_ref in manifest_skills:
            ...
            assert skill_dir.exists(), f"Manifest references non-existent skill: {skill_ref}"
    ...
    def test_command_references_in_manifest(self, sample_plugin_manifest):
        ...
        for command_ref in manifest_commands:
            ...
            assert command_ref.endswith('.md'), f"Command should be markdown: {command_ref}"

            # Should be in commands directory
            assert command_ref.startswith('./commands/'), f"Command should be in commands directory: {command_ref}"
