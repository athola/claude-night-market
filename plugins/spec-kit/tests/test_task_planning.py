...
class TestTaskPlanning:
    ...
    def test_task_phases_structure(self, sample_task_list):
        ...
        for i, expected_phase in enumerate(expected_phases[:len(actual_phases)]):
            assert actual_phases[i] == expected_phase, f"Phase {i} should be {expected_phase}"
    ...
    def test_task_dependency_structure(self, sample_task_list):
        ...
        for phase in sample_task_list:
            for task in phase["tasks"]:
                ...
                assert isinstance(task["dependencies"], list), f"Dependencies should be list, got {type(task['dependencies'])}"
        ...
        for task in setup_tasks:
            if task["dependencies"]:
                ...
                for dep_id in task["dependencies"]:
                    ...
                    assert not internal_dep, f"Setup task {task['id']} has internal dependency {dep_id}"
    ...
    def test_estimation_format(self, sample_task_list):
        ...
        estimation_pattern = r'^\d+[hm]$|^(\d+\.?\d*)\s*(hour|hours|day|days|week|weeks)s?$'

        for phase in sample_task_list:
            for task in phase["tasks"]:
                estimation = task["estimated_time"]
                assert re.match(estimation_pattern, estimation.lower()), f"Invalid time format: {estimation}"
    ...
    def test_parallel_execution_identification(self, sample_task_list):
        ...
        if setup_phase and len(setup_phase["tasks"]) > 1:
            assert len(parallel_groups) > 0, "Should identify parallel execution opportunities"
    ...
    def test_critical_path_identification(self, sample_task_list):
        ...
        start_tasks = [
            task_id for task_id, task_info in task_map.items()
            if not task_info["dependencies"]
        ]

        assert len(start_tasks) > 0, "Should have at least one task with no dependencies"
    ...
    def test_task_description_quality(self, sample_task_list):
        ...
        for phase in sample_task_list:
            for task in phase["tasks"]:
                ...
                assert len(description) > len(title), f"Description should be longer than title for {task['id']}"

                # Description should be specific
                vague_words = ["various", "multiple", "several", "some", "related"]
                description_lower = description.lower()
                vague_found = [word for word in vague_words if word in description_lower]

                # Allow some vague words but not too many
                assert len(vague_found) <= 1, f"Task description too vague: {description}"
    ...
    def test_task_id_consistency(self, sample_task_list):
        ...
        for phase in sample_task_list:
            for task in phase["tasks"]:
                assert re.match(id_pattern, task["id"]), f"Task ID doesn't match pattern: {task['id']}"
            ...
            for task in phase["tasks"]:
                for dep_id in task["dependencies"]:
                    ...
                    if dep_phase:
                        ...
                        phase_order = ["0 - Setup", "1 - Foundation", "2 - Core Implementation", "3 - Integration", "4 - Polish"]
                        current_index = phase_order.index(phase_name)
                        dep_index = phase_order.index(dep_phase)

                        assert dep_index <= current_index, f"Cross-phase forward dependency: {task['id']} depends on {dep_id}"
