...
usage: /fix-pr-wrapper [<pr-number> | <pr-url>] [--dry-run] [--commit-strategy single|separate|manual]
extends: "superpowers:receiving-code-review"
---

# Enhanced PR Fix Wrapper
...
/fix-pr-wrapper --dry-run

# Specific PR with separate commits
/fix-pr-wrapper 42 --commit-strategy separate
```
...
2. Verify all threads resolved
3. Check CI pipeline status
4. Notify reviewers of updates
...
Solution: Pull latest, resolve conflicts, re-run wrapper
```

## Migration from /fix-pr

```bash
# Original command (still supported)
/fix-pr

# Enhanced wrapper with superpowers
/fix-pr-wrapper

# Gradual migration
/fix-pr-wrapper --dry-run  # Preview capabilities
/fix-pr-wrapper           # Full adoption
```
