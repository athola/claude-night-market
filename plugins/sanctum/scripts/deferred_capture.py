#!/usr/bin/env python3
"""Deferred Capture - Create GitHub issues for deferred items.

Reference implementation of the leyline deferred-capture contract.
Other plugins create thin wrappers that delegate here or follow the
same CLI interface.

Usage:
    python deferred_capture.py --title TITLE --source SOURCE
                               --context CONTEXT [options]
    python deferred_capture.py --title "My idea" --source brainstorm
...
# ---------------------------------------------------------------------------
# Label taxonomy from the leyline deferred-capture contract
# ---------------------------------------------------------------------------
...
def find_duplicate(bare_title: str) -> dict | None:
    ...
    for issue in issues:
        ...
        # Strip [Deferred] prefix for comparison
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a GitHub issue for a deferred item.",
    )
    parser.add_argument(
        "--title",
        required=True,
    ...
    parser.add_argument(
        "--source", required=True, help="Origin skill (e.g. war-room, brainstorm)"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Bare title = stripped of [Deferred] prefix, used for dupe detection
    bare_title = (
