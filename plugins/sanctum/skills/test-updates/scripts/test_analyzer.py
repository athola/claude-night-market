...
class TestAnalyzer:
    ...
    def scan_for_test_gaps(self) -> dict[str, any]:
        ...
        results['source_files'] = [
            f for f in results['source_files']
    ...
    def _analyze_file_coverage(self, source_file: Path, test_files: list[Path]) -> dict:
        ...
        functions = [node.name for node in ast.walk(source_tree)
                     if isinstance(node, ast.FunctionDef)]
        classes = [node.name for node in ast.walk(source_tree)
        ...
        coverage_percentage = ((total_items - len(missing)) / total_items * 100) if total_items > 0 else 0
            ...
            for file_path in changed_files:
                if file_path.exists():
                    ...
                    diff_result = subprocess.run(
                        ['git', 'diff', '--name-status', 'HEAD~1', '--', str(file_path)],
                        check=False, capture_output=True,
                        text=True,
                        cwd=self.codebase_path
                    )
...
def main():
    ...
    parser.add_argument('--report', choices=['coverage', 'changes'],
                       default='coverage', help='Report type to generate')
