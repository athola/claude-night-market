...
class TestQualityChecker:
    ...
    def run_full_validation(self) -> dict[str, any]:
        ...
        results['quality_level'] = self._determine_quality_level(results['quality_score'])
    ...
    def run_static_analysis(self) -> dict[str, any]:
        ...
        if not self.test_path.exists():
            analysis['structure_issues'].append(
                QualityIssue('error', 'structure', f"Test file not found: {self.test_path}")
            )
            return analysis
    ...
    def _check_test_structure(self, tree: ast.AST, analysis: dict):
        ...
        if not imports:
            analysis['structure_issues'].append(
                QualityIssue('warning', 'structure', 'No imports found - may need pytest or unittest')
            )
                ...
                if not assertions:
                    ...
                    for assert_node in assertions:
                        if isinstance(assert_node.test, ast.Compare):
                            ...
                            if isinstance(assert_node.test.left, ast.Name) and \
                               assert_node.test.left.id == 'result':
                                analysis['assertion_issues'].append(
                                    QualityIssue(
                                        'warning', 'assertion',
    ...
    def _check_bdd_compliance(self, test_content: str, analysis: dict):
        ...
        for test_name in test_functions:
            test_match = re.search(f'def {test_name}\\(.*?\\):(.*?)(?=def |class |$)',
    ...
    def _check_documentation(self, test_content: str, analysis: dict):
        ...
        test_functions = re.finditer(r'def (test_[^(]+).*?"""(.*?)"""', test_content, re.DOTALL)
    ...
    def calculate_metrics(self) -> dict[str, any]:
        ...
        # Count assertions
        metrics['assertion_count'] = sum(1 for node in ast.walk(tree)
        ...
        # Documentation ratio
        docstring_lines = sum(len(doc.split('\n')) for doc in re.findall(r'"""(.*?)"""', content, re.DOTALL))
        total_lines = len(content.split('\n'))
        metrics['documentation_ratio'] = docstring_lines / total_lines if total_lines > 0 else 0
    ...
    def _generate_recommendations(self, results: dict) -> list[str]:
        ...
        if static['naming_issues']:
            recommendations.append("Use descriptive, snake_case test names that describe behavior")

        if static['assertion_issues']:
            recommendations.append("Write specific, meaningful assertions that verify behavior")

        if static['bdd_compliance']:
            recommendations.append("Add Given/When/Then clauses to test docstrings for BDD compliance")

        if static['documentation']:
            recommendations.append("Add comprehensive documentation to explain test scenarios")
        ...
        if dynamic['test_duration'] > 5:
            recommendations.append("Optimize test performance - tests should run quickly")
